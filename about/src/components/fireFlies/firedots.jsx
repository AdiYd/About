...
const FireDots = () => {
    ...
    useEffect(() => {
        ...
        for (let i = 1; i <= particleNum; i++) {
            ...
            const keyFrames = `@keyframes ${framesName} {
                from {
                    transform: translate3d(${Math.floor(Math.random() * 100)}vw, ${startPositionY}vh, 0);
                }
                to {
                    transform: translate3d(${Math.floor(Math.random() * 100)}vw, ${-startPositionY - Math.floor(Math.random() * 30)}vh, 0);
                }
            }`;
    ...
    return (
        <div ref={containerRef} className="circle-container"></div>
    );
};
